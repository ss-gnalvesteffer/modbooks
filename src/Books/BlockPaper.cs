...
namespace Books
{
    internal class BlockPaper : Block
    {
        ...
        public override bool OnBlockInteractStart(IWorldAccessor world, IPlayer byPlayer, BlockSelection blockSel)
        {
            ...
            {
                var BEBooks = (BlockEntityBooks)Entity;
        ...
        public override void OnBlockBroken(IWorldAccessor world, BlockPos blockPos, IPlayer byPlayer, float dropQuantityMultiplier = 0)
        {
            ...
            {
                var BEBooks = (BlockEntityBooks)beb;

                if (BEBooks.Unique)
                {
                    var UniqueBook = new ItemStack(api.World.BlockAccessor.GetBlock(blockPos));
                    var BookTree = new TreeAttribute();
        ...
        public override void OnBlockPlaced(IWorldAccessor world, BlockPos blockPos, ItemStack byItemStack)
        {
            ...
            if (world.Api is ICoreServerAPI)
            {
                ...
                if (be is BlockEntityBooks)
                {
                    ...
                    BEBooks.Unique = byItemStack.Attributes.GetBool(saveIsUnique);
